...
run_one() {
  ...
  python -m experiments.train_tess_basic \
    --root "$ROOT" \
    --dataset fnspid \
    --model legacy_multimodal_primitive_additive_gate \
...
case "$MODE" in
  additive_text_gate_scale1|text_gate_scale1)
    run_one additive_text_gate_scale1 text 1.0
    ;;
  additive_gt_gate_scale1|gt_gate_scale1)
    run_one additive_gt_gate_scale1 gt 1.0
    ;;
  all)
    run_one additive_text_gate_scale1 text 1.0
    run_one additive_gt_gate_scale1 gt 1.0
    ;;
  *)
    echo "MODE must be one of: additive_text_gate_scale1, additive_gt_gate_scale1, all" >&2
    exit 2
    ;;
esac
