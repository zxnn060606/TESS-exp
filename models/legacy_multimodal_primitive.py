"""Primitive-embedding migration of the legacy multimodal baseline.

The legacy baseline fuses one encoded time-series vector with one external
news/text embedding vector through a residual cross-modal attention block. This
RC2 version keeps that flow but replaces the external embedding service with
learnable embeddings over the four primitive label IDs available in each batch.
"""
...
class CrossModalAttention(nn.Module):
    """Legacy 2D cross-modal attention over one time vector and one primitive vector."""
    ...
    def forward(self, h_time: torch.Tensor, h_text: torch.Tensor) -> torch.Tensor:
        q = self.q_proj(h_time)
        k = self.k_proj(h_text)
        v = self.v_proj(h_text)
        scores = (q * k).sum(dim=-1, keepdim=True) / math.sqrt(self.embed_dim)
        probs = self.dropout(torch.softmax(scores, dim=-1))
        self.last_attention_scores = scores.detach()
        self.last_attention = probs.detach()
        attended = probs * v
        return self.out_proj(self.layer_norm(attended + h_time))


class LegacyMultimodalPrimitive(nn.Module):
    """No-gate primitive version of legacy `MultiModal_Baseline`."""
    ...
    def __init__(
        ...
        self.dynamic_fused_layer = CrossModalAttention(embed_dim=d_model, dropout=dropout3)
    ...
    def forward(
        ...
        primitive_embed = self.dynamic_fc(primitive_feature)
        fused = self.dynamic_fused_layer(h_ts, primitive_embed)

        dec_out = self.decoder_mlp(fused).unsqueeze(-1)
        dec_out = dec_out * stdev[:, 0, :].unsqueeze(1).repeat(1, self.pred_len, 1)
        dec_out = dec_out + means[:, 0, :].unsqueeze(1).repeat(1, self.pred_len, 1)
        return dec_out


class LegacyMultimodalPrimitiveGate(LegacyMultimodalPrimitive):
    """Margin-gated primitive variant with supervised soft reliability gates."""
    ...
    def __init__(
        ...
        self.gate_head = nn.Sequential(
            nn.Linear(primitive_emb_dim + margin_emb_dim, primitive_emb_dim),
            nn.PReLU(),
    ...
    def forward(
        ...
        gate_logits = self.gate_head(torch.cat([primitive_emb, margin_emb], dim=-1)).squeeze(-1)
        if primitive_mask is not None:
            gate_logits = gate_logits.masked_fill(~primitive_mask, -20.0)
        gate_weights = torch.sigmoid(gate_logits).unsqueeze(-1)

        gated_primitive = (primitive_emb * gate_weights).reshape(x.shape[0], -1)
        primitive_embed = self.dynamic_fc(gated_primitive)
        fused = self.dynamic_fused_layer(h_ts, primitive_embed)
...
class LegacyMultimodalPrimitiveDeltaGate(LegacyMultimodalPrimitiveGate):
    """Conservative gate that adds a small gated delta to the no-gate primitive path."""
    ...
    def __init__(
        ...
        super().__init__(
            ...
            dropout3=dropout3,
            embedding_dropout=embedding_dropout,
            margin_emb_dim=margin_emb_dim,
        )
        self.delta_gate_beta = float(delta_gate_beta)
        ...
        self.primitive_delta_proj = nn.Sequential(
            nn.Linear(primitive_emb_dim, primitive_emb_dim),
            nn.PReLU(),
        )
        self.delta_dynamic_fc = nn.Sequential(
            nn.Linear(4 * primitive_emb_dim, d_model),
            nn.PReLU(),
            nn.Dropout(dropout2),
        )
    ...
    def forward(
        ...
        primitive_emb = self._primitive_embeddings_3d(primitive_ids, primitive_mask)
        primitive_embed_base = self.dynamic_fc(primitive_emb.reshape(x.shape[0], -1))

        margin_emb = self.margin_projection(primitive_margins.unsqueeze(-1))
        time_context = self.time_gate_projection(h_ts).unsqueeze(1).expand(-1, primitive_emb.shape[1], -1)
        ...
        primitive_delta = self.primitive_delta_proj(primitive_emb)
        gated_delta = (primitive_delta * gate_weights).reshape(x.shape[0], -1)
        primitive_delta_embed = self.delta_dynamic_fc(gated_delta)
        primitive_embed = primitive_embed_base + self.delta_gate_beta * primitive_delta_embed
